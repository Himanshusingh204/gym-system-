...
const STATS = [
  { label: 'Active Members', value: 10000, suffix: '+', icon: Users },
  { label: 'Partner Gyms', value: 500, suffix: '+', icon: Globe2 },
  { label: 'Pro Trainers', value: 1000, suffix: '+', icon: Dumbbell },
  { label: 'Cities Covered', value: 50, suffix: '+', icon: CalendarCheck },
];
            ...
            {STATS.map(({ label, value, suffix, icon: Icon }, i) => (
              <Reveal key={label} delay={i * 100}>
                <div className="card-hover h-full flex flex-col items-center text-center p-8!">
                  ...
                  <p className="mt-2 text-xs font-bold uppercase tracking-wider text-[var(--color-muted)]">{label}</p>
                </div>
              </Reveal>
            ))}
          </div>
            ...
            {PLANS.map((plan, i) => (
              <Reveal key={plan.name} delay={i * 120} className="h-full">
                <div className={`relative h-full rounded-2xl transition-all duration-300 hover:-translate-y-2 ${
                  ...
                  <h3 className={`text-2xl font-extrabold ${plan.highlight ? 'text-white' : ''}`}>{plan.name}</h3>
                  <p className={`text-sm mt-1 ${plan.highlight ? 'text-white/70' : 'text-[var(--color-muted)]'}`}>{plan.tagline}</p>

                  <div className="flex items-end gap-1 mt-6 mb-8">
                    <span className={`text-sm font-semibold mb-1.5 ${plan.highlight ? 'text-white/70' : 'text-[var(--color-muted)]'}`}>₹</span>
                    <span className="text-5xl font-extrabold tracking-tight">{plan.price.toLocaleString('en-IN')}</span>
                    <span className={`text-sm font-semibold mb-1.5 ${plan.highlight ? 'text-white/70' : 'text-[var(--color-muted)]'}`}>/month</span>
                  </div>

                  <ul className="space-y-3 mb-8">
                    {plan.features.map((f) => (
                      <li key={f} className={`flex items-start gap-3 text-sm ${plan.highlight ? 'text-white/85' : 'text-[var(--color-charcoal)]'}`}>
                        <span className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center shrink-0 ${
                          plan.highlight ? 'bg-white/20 text-white' : 'bg-[var(--color-primary)]/10 text-[var(--color-primary)]'
                        }`}>
      ...
      <section className="section-pad bg-[var(--color-base-dark)]/50 dark:bg-white/[0.03]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="max-w-3xl mx-auto text-center mb-16">
            ...
            <h2 className="section-title mt-4">Trusted by 500+ Gyms</h2>
